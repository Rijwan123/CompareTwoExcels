...
def write_outputs(res, outdir, sheet):
    ...
    # summary.json (UPDATED: include missing rows also)
    summary = {
        "sheet": sheet,
        "stats": res["stats"],
...
def print_summary_json_for_tosca(summary: dict, summary_path: str) -> int:
    ...
    result = "PASS"
    if mismatches > 0 or miss_f1 > 0 or miss_f2 > 0:
        result = "FAIL"
    ...
    print(f"EXTRA_COLUMNS_FILE1={stats.get('extra_columns_file1', 0)}")
    print(f"EXTRA_COLUMNS_FILE2={stats.get('extra_columns_file2', 0)}")
...
#python compare_excel_v5.py --file1 S5_ColChange_Reference.xlsx --file2 S5_ColChange_Target.xlsx